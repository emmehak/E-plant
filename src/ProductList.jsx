...
function ProductList({ onContinueShopping }) {
  ...
  const styles = {
    ...
    addButton: (isInCart) => ({
      backgroundColor: isInCart ? "#dddddd" : "#abdbe3",
  ...
  const handleAddToCart = (plant) => {
    ...
    if (!isInCart) {
      dispatch(addItem({ ...plant, quantity: 1, id: plant.name }));
    }
  };
    ...
    <div>
      ...
      <div style={styles.navbar}>
        ...
        <div style={styles.navRight}>
          <span onClick={() => setShowCart(false)} style={styles.navLink}>
            Plants
          </span>
          <span onClick={() => setShowCart(true)} style={styles.navLink}>
        ...
        <div style={styles.productGrid}>
          {plantsArray.map((category) => (
            <div key={category.category}>
              ...
              <div style={styles.plantCategory}>
                {category.plants.map((plant, index) => {
                  const isInCart = cartItems.some(
                    (item) => item.name === plant.name
                  );
                  return (
                    <div key={plant.name + index} style={styles.plantCard}>
                      <img
                        src={plant.image}
                        alt={plant.name}
                      ...
                      <button
                        ...
                        style={styles.addButton(isInCart)}
                        disabled={isInCart}
                      >
